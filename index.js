...
const marketPayment = require("./server/marketing/main/paymentGateway.controller");
...
app.post(
  "/marketing/wallet/initiate_payment",
  verifyJWT,
  (req, res) =>
    console.log("Initiate Payment API called") ||
    marketPayment.initiatePayment(req, res, config)
);

app.post("/update-recharge-status", marketPayment.updatePaymentStatusWallet);
...
app.post("/generate-upi-link", (req, res) => {
  console.log("Generate UPI Link API called");
  easebuzzController.createUpiLink(req, res, config);
});
