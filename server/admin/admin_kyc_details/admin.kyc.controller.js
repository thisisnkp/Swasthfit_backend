...
exports.createAdminKYC = async (req, res) => {
    try {
        ...
        res.status(201).json({
            meta: { "correlation-id": uuidv4(), "code": "201", "message": "KYC record created successfully" },
            data: newRecord
        });
    } catch (error) {
...
exports.getAdminKYC = async (req, res) => {
    try {
        ...
        if (!record) return res.status(404).json({ errors: [{ code: "NOT_FOUND", message: "Record not found" }] });
        res.json(record);
    } catch (error) {
...
exports.updateAdminKYC = async (req, res) => {
    try {
        const updated = await AdminKYCDetails.update(req.body, { where: { id: req.params.id } });
        if (!updated[0]) return res.status(404).json({ errors: [{ code: "NOT_FOUND", message: "Record not found" }] });
...
exports.deleteAdminKYC = async (req, res) => {
    try {
        const deleted = await AdminKYCDetails.destroy({ where: { id: req.params.id } });
        if (!deleted) return res.status(404).json({ errors: [{ code: "NOT_FOUND", message: "Record not found" }] });
