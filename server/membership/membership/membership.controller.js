    ...
    try {
      ...
      if (!status || (status !== "Active" && status !== "Inactive")) {
        return res.status(400).json({
          errors: [{ code: "INVALID_STATUS", message: "Status must be 'Active' or 'Inactive'" }],
          correlationId,
        });
      }
...
exports.createMembership = async (req, res) => {
    try {
        ...
        const {
            user_id, gym_id, membership_plan_id, start_date, end_date, price,
            membership_type, status, payment_method, transaction_id, coupon_id
        } = req.body;

        // Validate required fields
        if (!user_id || !membership_plan_id || !start_date || !end_date || !price || !membership_type || !status || !payment_method) {
            return res.status(400).json({
...
exports.updateMembership = async (req, res) => {
    try {
        ...
        const membership = await Membership.findOne({ where: { id, deleted_at: null } });
        if (!membership) {
            return res.status(404).json({
                errors: [{ code: "NOT_FOUND", message: "Membership not found" }],
            });
