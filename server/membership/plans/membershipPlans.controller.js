...
exports.getMembershipPlans = async (req, res) => {
  try {
    const { role, id: userId } = req.user; // Assuming 'id' and 'role' are in the JWT payload
    let conditions = {};

    if (role === 'gym') {
      conditions = { createdBy: 'gym', gymId: userId }; // Gyms see their own plans
    } else if (role === 'admin') {
      conditions = { createdBy: 'admin' }; // Admins see plans created by admins
    }
    ...
    if (plans.length === 0) {
      return res.status(404).send({ message: "No membership plans found." });
    }
...
const updateMembershipPlan = async (req, res) => {
  try {
    ...
    if (!plan) {
      return res.status(404).send({ message: "Membership plan not found or access denied." });
    }
