...
const RestVendors = require("./Restaurants/restVendor.model");
...
const Report = require("./market_report.model");
...
Vendor.hasMany(WalletTransaction, {
...
CampaignAnalysis.belongsTo(Campaign, {
  foreignKey: "campaign_id",
  as: "campaign",
});
...
VendorWalletPayments.belongsTo(Vendor, {
  foreignKey: "vendor_id",
  as: "vendor",
});
...
// Change the alias in the CampaignAnalysis association to be unique
CampaignAnalysis.belongsTo(Campaign, {
  foreignKey: "campaign_id",
  as: "campaign_details", // Changed from 'campaign' to 'campaign_details'
});
...
const db = {
  ...
  VendorWallet,
  foodItems,
  RestVendors, // Add Menu model
  Gym,
  Report,
  GymOwner, // Add Gym model
};
