  ...
  try {
    ...
    for (const post of scheduledPosts) {
      ...
      if (scheduledTime > currentTime) {
        console.log(
          `Post ID ${post.id} is not ready to be posted yet. Scheduled for: ${scheduledTime}`
        );
        continue;
      }
      ...
      console.log(
        `Generated JWT Token for vendor_id: ${vendor_id}, module_type: ${module_type}`
      );
      ...
      try {
        if (platform === "facebook") {
          ...
          await axios.post(
            "http://localhost:4001/marketing/site/apis/facebook/post",
            requestData,
            {
          ...
          await axios.post(
            "http://localhost:4001/marketing/site/apis/instagram/post",
            requestData,
            {
              headers: { Authorization: `Bearer ${token}` },
            }
