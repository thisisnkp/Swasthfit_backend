...
router.post("/verify", verifyJWT, getVerify.generateToken);

const getVendorDetails = require("./function/Wallet/getVendorData.controller");
router.post("/getVendor", verifyJWT, getVendorDetails.getVendorDetails);
...
router.get(
  "/dashboardAnalytics",
  verifyJWT,
  getDashboardData.getWalletAnalytics
);
...
router.get(
  ...
  verifyJWT,
  productController.getFoodItemsByRestaurantId
);
...
router.get(
  "/vendor-analytics",
  verifyJWT,
  vendorAnalyticsController.getVendorAnalytics
);
...
router.get(
  "/walletTransaction",
  verifyJWT,
  walletController.getWalletTransactionsByVendor
);
router.get(
  "/walletRecharge",
  verifyJWT,
  walletController.getAllVendorWalletPayments
);
router.post(
  "/payWalletRecharge",
  verifyJWT,
  walletController.createVendorWalletPayment
);
...
router.post(
  "/deductWallet",
  verifyJWT,
  walletDeductionController.deductFromWallet
);
router.post(
  "/addMoney",
  verifyJWT,
  walletDeductionController.rechargeVendorWallet
);
...
router.post(
  "/getquota",
  verifyJWT,
  contactController.authenticateAndSubmitContact
);
...
router.get(
  "/facebook/callback",
  facebookAuthController.facebookCallback,
  facebookAuthController.facebookSuccess
);
...
router.get(
  "/facebook/check-auth",
  verifyJWT,
  facebookAuthController.checkFacebookAuth
);

router.get(
  "/facebook/get-user",
  verifyJWT,
  facebookAuthController.getUserByToken
);
...
router.post(
  "/instagram/post",
  verifyJWT,
  instagramPostController.postToInstagram
);
...
router.post(
  "/schedule/post",
  verifyJWT,
  schedulePostController.createFacebookPost
);
...
router.get(
  "/facebook/getPosts",
  verifyJWT,
  getPostByStatusController.getPostsByStatus
);

const getPostByIdController = require("./function/ManageSocialMedia/getAllPagesOfFacebook.controller");
router.get(
  "/facebook/getAllPages",
  verifyJWT,
  getPostByIdController.getAllFacebookPages
);

const {
  exchangeToken,
} = require("./function/AuthenticationFacebook/longLivedToken");

// ... existing code ...
router.post("/facebook/exchange-token", verifyJWT, exchangeToken);
// ... existing code ...
...
router.get(
  "/report",
  verifyJWT,
  reportController.getCampaignAnalysis // Change this line to match the exported function name
);
