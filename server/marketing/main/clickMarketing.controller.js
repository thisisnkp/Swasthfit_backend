...
const Report = db.Report; // Add Report model import
...
const trackUserAction = async (req, res) => {
  ...
  if (!validActions.includes(action)) {
    return res.status(400).json({
      message:
        "Invalid action. Must be one of: single-view, add-to-cart, add-to-wish-list, buy",
    });
  }
  ...
  try {
    ...
    for (const campaign of campaigns) {
      ...
      if (isMatch) {
        ...
        // Find or create campaign analysis record
        let analysis = await db.CampaignAnalysis.findOne({
          where: { campaign_id },
        });

        if (!analysis) {
          analysis = await db.CampaignAnalysis.create({
            campaign_id,
            vendor_id,
            click: 0,
            reach: 0,
        ...
        // Update analysis based on action
        const updateValues = {};
        switch (action) {
          case "buy":
            updateValues.buy = analysis.buy + 1;
            break;
          case "single-view":
          case "add-to-cart":
          case "add-to-wish-list":
            updateValues.click = analysis.click + 1;
            break;
        }

        // Update campaign analysis
        await analysis.update(updateValues);

        // Create market report entry
        await Report.create({
          report_id: analysis.id,
          product_id: product_id,
          remark: `Action performed: ${action}`,
        });

        // Continue with existing wallet transaction logic
        ...
        if (action === "buy") {
          ...
          ["single-view", "add-to-cart", "add-to-wish-list"].includes(action)
        ) {
          newClick = 1;
        }
        ...
        if (!transaction) {
          ...
          // Create market report for click action
          if (newClick > 0) {
            await Report.create({
              report_id: transaction.id, // Using transaction ID as report_id
              product_id,
              remark: `Click action: ${action}`,
            });
          }
        } else {
          const updateValues = {};
          ...
          updateValues.cost =
            parseFloat(transaction.cost) + parseFloat(bidAmount);
