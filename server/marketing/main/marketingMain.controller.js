...
const { Sequelize } = require("../../../sequelize"); // Add Sequelize import here
...
const searchProductsByName = async (module_type, productName) => {
  try {
    ...
    console.error(
      `Error searching products by name for ${module_type}:`,
      error
    );
    return [];
  }
...
const checkCampaignInRadius = async (req, res) => {
  const { latitude, longitude, product_name, module_type, product_id } =
    req.body;
  ...
  if (!product_name && !product_id) {
    return res
      .status(400)
      .json({ message: "Either product_name or product_id is required" });
  }
  ...
  try {
    ...
    if (module_type === "ecom") {
      ...
      // Modify the campaign search query
      if (city) {
        console.log("Searching campaigns for city:", city);
        ...
        // If no campaigns found by city, try radius search
        if (campaigns.length === 0) {
          console.log("No campaigns found by city, trying radius search");
          campaigns = await Campaign.findAll({
            where: {
              module_type,
              product_id: { [Op.in]: productIds },
          ...
          console.log("Found campaigns by radius:", campaigns.length);
        }
      }
    } else {
    ...
    const campaignProductIds = [
      ...new Set(campaigns.map((campaign) => campaign.product_id)),
    ];
      ...
      if (isMatch) {
        ...
        const campaignWithDetails = {
          ...
          productDetails: productDetail
            ? formatProductDetails(module_type, productDetail)
            : null,
        };
        ...
        if (!wallet) {
          return res.status(404).json({
            message: `Vendor wallet not found for vendor ${vendor_id}.`,
          });
        }
        ...
        if (!existingTransaction) {
          ...
          const newCost =
            parseFloat(existingTransaction.cost) + parseFloat(bidAmount);
        ...
        wallet.walletAmount =
          parseFloat(wallet.walletAmount) - parseFloat(bidAmount);
        await wallet.save();
    ...
    return res.status(200).json({
      ...
      matchedCampaigns,
      detectedCity: city,
      searchedProducts: product_name
        ? formatSearchedProducts(module_type, searchedProducts)
        : null,
