...
exports.initiate_payment = function (req, config, res) {
  var data = req.body; // Fixed undefined `req`
  ...
  function checkArgumentValidation(data, config) {
    ...
    if (!data.phone.trim() || !util.validate_phone(data.phone)) { // Fixed phone validation logic
      res.json({
        status: 0,
  ...
  function geturl(env) {
    let url_link; // Declared `url_link` variable
    if (env == "test") {
